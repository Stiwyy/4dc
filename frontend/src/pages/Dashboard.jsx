...
import { LogOut, UserPlus, MessageSquare, Send, User, Lock, Loader2 } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            await chatAPI.sendMessage(selectedChat.chatId, user.uid, encrypted);
                        ...
                        <div className="space-y-4 p-2">
                            ...
                            {myRequests.length > 0 && (
                                <div className="mb-4 animate-in fade-in slide-in-from-left-4">
                                    <h3 className="text-xs text-emerald-500 uppercase font-bold mb-2 px-1">Incoming Requests</h3>
                            ...
                            <div>
                                ...
                                {myContacts.length === 0 && <div className="text-xs text-neutral-600 px-1">No secure links yet.</div>}
                                {myContacts.map((contactId) => (
                                    <div key={contactId} onClick={() => handleStartChat(contactId)} className="group p-3 rounded-lg hover:bg-white/5 cursor-pointer flex items-center gap-3 border border-transparent hover:border-emerald-500/20 transition-all">
                                        <div className="h-8 w-8 rounded-full bg-neutral-800 flex items-center justify-center group-hover:bg-emerald-900/30 transition-colors">
                                            <User className="w-4 h-4 text-neutral-400 group-hover:text-emerald-500" />
                                        </div>
                                        <div className="overflow-hidden">
                                            <div className="font-bold text-sm text-neutral-300 group-hover:text-white truncate w-40">{contactId}</div>
                                            <div className="text-[10px] text-neutral-500 group-hover:text-emerald-500">Secure Link Ready</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <>
                            {myContacts.length === 0 && (
                                <div className="text-center mt-10 p-4">
                                    ...
                                    <div className="text-xs text-neutral-500">No active keys.<br/>Add a contact to start.</div>
                                </div>
                            )}
                            {myContacts.map((contactId) => (
                                <div
                                    ...
                                    className={`p-3 rounded-lg cursor-pointer transition-all border ${selectedChat?.partnerId === contactId ? 'bg-white/5 border-emerald-500/30 shadow-[0_0_15px_-3px_rgba(16,185,129,0.1)]' : 'hover:bg-white/5 border-transparent'}`}
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        <div className={`font-bold text-sm ${selectedChat?.partnerId === contactId ? 'text-emerald-400' : 'text-neutral-300'}`}>
                  ...
                  <span className="relative flex h-3 w-3 mr-3">
                    ...
                    <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span>
                            ...
                            {activeChatMessages.map((msg) => {
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                                        <div className={`max-w-[70%] rounded-2xl px-4 py-2 text-sm shadow-lg backdrop-blur-sm border ${
                                            isMe
                        ...
                        <div className="p-4 border-t border-neutral-800 bg-neutral-900/80 z-10 backdrop-blur-md">
                            <form onSubmit={handleSendMessage} className="flex gap-2 max-w-4xl mx-auto">
                                ...
                                />
                                <Button type="submit" className="h-full aspect-square p-0 px-4 rounded-lg bg-emerald-600 hover:bg-emerald-500 shadow-[0_0_15px_-3px_rgba(16,185,129,0.3)] border-0">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-neutral-600 gap-4">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-sm mt-1">Select a contact from the network<br/>to initialize encrypted handshake.</p>
                        </div>
                    </div>
                )}
            </div>
