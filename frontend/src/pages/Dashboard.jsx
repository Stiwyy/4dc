...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        const unsubChat = onSnapshot(q, (snapshot) => {
            ...
            const decryptedMsgs = msgs.map(m => {
                let content = "⚠️ Decryption Error";
        ...
        selectedChat.members.forEach(memberId => {
            ...
            const unsub = onSnapshot(doc(db, "users", memberId), (docSnap) => {
                if (docSnap.exists() && docSnap.data().username) {
                    setMemberNames(prev => ({ ...prev, [memberId]: docSnap.data().username }));
                }
            });
            unsubs.push(unsub);
        });
    ...
    const handleCreateGroup = async (e) => {
        ...
        try {
            ...
            setSelectedChat({ chatId: res.chatId, type: 'group', name: newGroupName, members: [user.uid, ...selectedGroupMembers] });
        } catch (err) {
            console.error(err);
    ...
    const handleStartChat = async (cid) => {
        try {
            ...
            setSelectedChat({ chatId: res.chatId, partnerId: cid, type: 'direct', members: [user.uid, cid] });
            setActiveTab('chats');
                        ...
                        <div className="space-y-2">
                            ...
                            {myChats.map((chat) => {
                                if (chat.type === 'group') {
                                    ...
                                    return <ContactItem key={chat.chatId} contactId={partnerId} isActive={selectedChat?.chatId === chat.chatId} onClick={() => setSelectedChat({ chatId: chat.chatId, partnerId, type: 'direct', members: chat.members })} />;
                                }
                            })}
                        </div>
                    )}
                                    ...
                                    <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                                        ...
                                        <div id={msg.id} className="group relative flex items-end gap-2 max-w-[75%]">
                                            ...
                                            <div className={`rounded-2xl px-4 py-2 text-sm shadow-lg backdrop-blur-sm border relative group/msg ${
                                                ...
                                                <div className={`text-[9px] mt-1 font-mono flex items-center gap-1 select-none ${isMe ? 'text-emerald-400/60 justify-end' : 'text-neutral-500 justify-end'}`}>
                                                    {formatTime(msg.sentAt)}
                                                    {isMe && <span>✓</span>}
                                                </div>

                                                {isMe && editingMessageId !== msg.id && (
                                                    <div className="absolute -top-3 right-0 hidden group-hover/msg:flex gap-1 bg-neutral-900 border border-neutral-800 rounded px-1 py-0.5 shadow-xl z-20">
